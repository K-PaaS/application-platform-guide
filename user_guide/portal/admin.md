### [Index](https://github.com/K-PaaS/Guide/blob/master/README.md) > [AP User Guide](../README.md) > 운영자 포털
...
1. [문서 개요](#1)
    ...
    * [1.2. 범위](#1.2)
    * [1.3. 운영자 포탈 설치](#1.3)
2. [K-PaaS AP 운영자 포탈 시작](#2)
    * [2.1. K-PaaS AP 운영자 포탈 로그인](#2.1)
    * [2.2. K-PaaS AP 운영자 포탈 로그](#2.2)
3. [K-PaaS AP 운영자 포탈 대시보드](#3)
    * [3.1. 대시보드 정보 조회](#3.1)
4. [운영 관리 메뉴](#4)
    * [4.1. 조직 및 공간 조회](#4.1)    
    * [4.2. 클라이언트](#4.2)    
...
본 문서는 JAVA로 작성된 K-PaaS AP 운영자 포탈 웹 사이트를 사용하고 관리하는 방법에 대해 설명한다.

### <a name="1.2"/>   1.2. 범위

이미 설치된 K-PaaS AP 운영자 포탈 웹 사이트 사용하는 방법에 대해 기술하였다.

### <a name="1.3"/>  1.3. 운영자 포탈 설치

본 사용 가이드는 설치에 대한 내용은 기술하지 않았다.

 운영자 포탈 설치는 [AP Portal 배포 가이드 문서](https://github.com/K-PaaS/Guide-3.0-Penne-/blob/v3.5/Install-Guide/Portal/PaaS-TA%20Portal%20%EC%84%A4%EC%B9%98%EA%B0%80%EC%9D%B4%EB%93%9C.md)를 참고한다.

## <a name="2"/>2.  K-PaaS AP 운영자 포탈 시작

K-PaaS AP 운영자 포탈 웹 사이트에 접속한다.

### <a name="2.1"/>  2.1. K-PaaS AP 운영자 포탈 로그인

1. K-PaaS AP 운영자 포탈에 접속하면 인증을 위한 로그인 화면이 나타난다.
![01]

2. 아이디와 비밀번호를 입력후 "로그인" 버튼을 클릭한다.
![02]

### <a name="2.2"/>  2.2. K-PaaS AP 운영자 포탈 로그아웃

1. 로그인 후 메인화면 ① "로그아웃" 버튼을 클릭하여 로그아웃을 한다.
![03]

## <a name="3"/>3.  K-PaaS AP 운영자 포탈 대시보드

K-PaaS AP에 등록된 조직, 공간, APP, 사용자 통계 정보를 보여준다.
...
K-PaaS AP 포탈의 운영에 필요한 관리를 수행하기 위한 메뉴이다.<br>
...
**관리자에게 클라이언트 조회,등록,삭제 기능을 제공한다.<br>**
...
**관리자가 카탈로그의 앱 템플릿, 앱 개발환경, 앱서비스 조회,등록,수정,삭제 기능을 제공한다.<br>**
...
5. 썸네일은 사용자가 등록하고자 하는 이미지를 지정하여 선택한다. 
     > **썸네일 적용 시 'K-PaaS AP 사용자'에서 카탈로그 서비스 리스트 및 서비스 등록 시 이미지를 확인 할 수 있다.**<br>
6. 문서 URL을 넣어준다. 
     > (예) https://github.com/K-PaaS/ap-pipeline-ui <br>
     > **문서 URL 적용 시 'K-PaaS AP 사용자'카탈로그 서비스 등록 '문서보기' 버튼 활성화 된다.**
...
9. 대시보드 사용 시 "Y" 를 필수 설정한다. 대시보드 "N" 설정시 대시보드 버튼이 비 활성화 된다.
     > **대시보드 적용 시 'K-PaaS AP 사용자'에서 카탈로그 앱 서비스 "대시보드" 버튼 활성화 된다**
     > ![pipeline-app-dashboard]
10. 그 외 공개여부, 요약, 설명, 카탈로그 관리 코드를 입력한다. "저장" 버튼을 클릭하여 서비스 등록을 완료한다.
     > ![pipeline]
...
![27]
     ...
     > ![27-1]
4. ①"관리" 버튼 클릭 후 "가입 해지"를 클릭하여 CF 로그인을 차단한다. 비승인일시 "가입 승인"을 클릭하여 CF 로그인을 승인한다.<br>
![30]
![31]
![31-2]
...
**관리자가 K-PaaS AP 포탈의 코드를 관리한다.<br>**

1. K-PaaS AP 포탈에서 사용하는 코드를 코드그룹별로 조회한다. 사용여부, 검색유형, 검색어로도 조회가 가능하다.<br>
![39]
...
**관리자가 K-PaaS AP 포탈의 설정 정보를 조회,등록,수정 기능을 제공한다.<br>**

1. K-PaaS AP 포탈에서 사용하는 설정 정보들을 조회한다.
![60]

2. K-PaaS AP 포탈에서 사용하는 설정 정보들을 수정하고 저장한다.
![62]

### <a name="4.8"/>  4.8. 할당량 관리

**관리자가 K-PaaS AP 포탈의 조직 및 공간 할당량을 조회,등록,수정,삭제를 기능을 제공한다.<br>**
...
5. "공간 할당량 등록" 버튼을 클릭하여 공간 할당량을 등록한다.<br>
![58]
...
1. K-PaaS AP 포탈에서 사용하는 분활 관리 내역을 조회한다.<br>
![isolationSegments01]
...
K-PaaS AP 포탈의 서비스 관리를 수행하기 위한 메뉴이다.<br>

### <a name="5.1"/>  5.1. 빌드팩

**관리자가 빌드팩의 조회,수정,사용유무 및 Lock유무를 관리한다.<br>**

1. K-PaaS AP 포탈에서 사용하는 빌드팩 정보들을 조회한다.<br>
![50]
...
1. K-PaaS AP 포탈에서 사용하는 서비스 브로커 내역을 조회한다.<br>
![54]
...
1. K-PaaS AP 포탈에서 사용하는 서비스 내역을 조회한다.<br>
![63]
...
K-PaaS AP 포탈의 보안 관리를 수행하기 위한 메뉴이다.<br>
    
### <a name="6.1"/>  6.1 시큐리티 그룹

**관리자가 staging, running 규칙을 사용하여 앱 및 작업 준비하고 특정 spaces에 대한 접근제어 관리한다.**<br>

1. K-PaaS AP 포탈에서 사용하는 시큐리티 그룹 정보들을 조회한다.<br>
![securitygroups01]

2. ①"시큐리티 그룹 등록" 버튼을 클릭하여 시큐리티 그룹을 등록한다. <br>
![securitygroups02]

3. 시큐리티 그룹 등록 팝업에 이름을 입력(필수)하고 staging default설정(필수), running default설정(필수) 체크박스를 선택한다. 팝업의 탭의 내용 입력 부분에 설명, 규칙(필수), 로그, 포트, 프로토콜을 입력한다. ① "+" 를 클릭하여 탭을 증가시킨다. ② "탭 삭제" 버튼을 클릭하여 탭을 삭제한다.<br>
![securitygroups03]

4. 시큐리티 그룹 등록 팝업에서 ①"파일 내보내기" 버튼을 클릭하여 시큐리티 그룹 정보를 ②.txt 파일로 확인 할 수 있다.
![securitygroups04]

5. 시큐리티 그룹 등록 팝업에서 ①"파일 불러오기" 버튼을 클릭하여 시큐리티 그룹 정보를 ②.txt 파일로 확인 할 수 있다. 불러온 파일은 추가 된 새 탭에서 확인 할 수 있다.
![securitygroups05]
